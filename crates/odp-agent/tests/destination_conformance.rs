...
struct PinnedStub(std::sync::Arc<Stub>);

#[async_trait::async_trait]
impl Transport for PinnedStub {
    async fn send(
        &self,
        _: HttpRequest,
    ) -> Result<odp_directory::HttpResponse, odp_directory::TransportError> {
        panic!("SecureTransport must use pinned destinations");
    }
    ...
    async fn send_to(
        ...
        assert!(
            addresses
                .iter()
                .all(|address| address.port() == url.port_or_known_default().unwrap())
        );
        if url.host_str() == Some("93.184.216.34") {
...
async fn reaches_a_public_destination() {
    ...
    let guarded = SecureTransport::new(std::sync::Arc::new(PinnedStub(inner.clone())));
...
async fn permits_loopback_only_for_local_development() {
    ...
    let guarded =
        SecureTransport::for_local_development(std::sync::Arc::new(PinnedStub(inner.clone())));
