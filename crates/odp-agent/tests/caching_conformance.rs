...
#[tokio::test]
async fn shared_cache_is_isolated_by_default_and_shared_only_explicitly() {
    let cache = Arc::new(MemoryCache::new());
    let first = Stub::serving(OFFERING_PAGE);
    let second = Stub::serving(OFFERING_PAGE);
    client(&first)
        .with_cache(cache.clone())
        .inspect()
        .await
    ...
    let first = client(&first)
        .with_cache(cache.clone())
        .with_cache_partition("same-identity");
    let second = client(&second)
        .with_cache(cache)
        .with_cache_partition("same-identity");
    first.inspect().await.unwrap();
    assert_eq!(second.inspect().await.unwrap().freshness, Freshness::Fresh);
    assert_eq!(
        second.clone().inspect().await.unwrap().freshness,
        Freshness::Fresh
    );
}

#[tokio::test]
async fn manual_continuations_do_not_assume_catalog_freshness() {
    let stub = Stub::serving(OFFERING_PAGE);
    let client = client(&stub);
    client
        .continue_offerings("/odp/offerings?cursor=search")
        .await
        .unwrap();
    client
        .continue_offerings("/odp/offerings?cursor=search")
...
#[tokio::test]
async fn stale_post_searches_are_repeated_without_get_preconditions() {
    let stub = Stub::new(|request| {
        ...
        Ok(with_headers(
            ...
            OFFERING_PAGE,
            ODP_JSON,
            &[("cache-control", "max-age=0"), ("etag", "search-v1")],
        ))
    });
    let client = client(&stub);
    let request = odp_core::OfferingSearchRequest {
