...
/// ROLE-03 and SVC-27: an index echoing a Service Document has not verified it, so those members
/// do not travel with the record. An Agent reads them from the Service itself or not at all.
#[tokio::test]
async fn drops_the_members_the_directory_cannot_vouch_for() {
    let stub = Stub::serving(one(&format!(
    ...
    assert_eq!(
        carried,
        ["future_member"],
        "only unknown members ride along"
    );
...
async fn refuses_a_prefix_the_directory_would_refuse() {
    ...
    for request in [
        ...
        assert!(client(&stub).suggest(&request).await.is_err());
    }
...
async fn refuses_a_trust_filter_this_version_does_not_define() {
    ...
    for trust in [
        ...
        let error = client(&stub)
            .search(&filtered(ServiceFilters {
                trust: trust.clone(),
                ..ServiceFilters::default()
            }))
            .await
