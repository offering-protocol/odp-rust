...
async fn leaves_a_continuation_a_caller_can_resume_from() {
    ...
    let resumed = client(&stub).continue_search_services(&next).await.unwrap();
...
#[tokio::test]
async fn defaults_to_sixteen_pages() {
    ...
    assert_eq!(services.len(), 160);
    assert_eq!(stub.count(), 16);
}
...
async fn refuses_a_continuation_that_is_not_a_reference() {
    ...
    assert!(
        client(&stub)
            .continue_search_services("https://[not-an-address")
            .await
            .is_err()
    );
}
...
async fn resolves_a_relative_continuation_against_the_directory() {
    let stub = Stub::serving(page(&[], ""));
    client(&stub)
        .continue_search_services("/v1/services/search?cursor=c2")
        .await
        .unwrap();
    assert_eq!(
        stub.last().url,
