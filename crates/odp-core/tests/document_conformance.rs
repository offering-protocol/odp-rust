...
#[test]
fn accepts_compatible_minor_versions_without_rewriting_the_input() {
    for version in ["1.0", "1.1", "1.7"] {
        let document = support::amend(
            support::service_document(),
            serde_json::json!({"odp_version":version}),
        );
        let parsed = odp_core::parse_service_document(&support::encode(&document)).unwrap();
        assert_eq!(parsed.odp_version, version);
    }
}

#[test]
fn enforces_document_depth_before_discarding_unknown_members() {
    let mut document = support::service_document();
    let mut nested = serde_json::json!(0);
    for _ in 0..8 {
        nested = serde_json::json!({"nested":nested});
    }
    document["extension"] = nested;
    assert!(odp_core::parse_service_document(&support::encode(&document)).is_err());
    assert!(odp_core::parse_agent_service_document(&support::encode(&document)).is_err());
}

#[test]
fn depth_counts_containers_not_scalar_fields() {
    let mut value = serde_json::json!({"leaf":1});
    for _ in 0..7 {
        value = serde_json::json!({"nested":value});
    }
    assert!(odp_core::validate_json_depth(&value, 8).is_ok());
    assert!(odp_core::validate_json_depth(&value, 7).is_err());
}

#[test]
fn unsupported_branding_does_not_reject_the_service() {
    for member in ["icon", "logo"] {
        let mut document = support::service_document();
        document["branding"] = serde_json::json!({"icon":{"src":"/icon.png","type":"image/png"},"logo":{"src":"/logo.png","type":"image/png"}});
        document["branding"][member]["type"] = serde_json::json!("image/future");
        let parsed = odp_core::parse_agent_service_document(&support::encode(&document)).unwrap();
        assert!(parsed.branding.is_none());
    }
}

#[test]
fn unsupported_schema_reference_only_removes_attributes() {
    let document = support::amend(
        support::offering(),
        serde_json::json!({"schema":{"url":"/schema","future":true},"attributes":{"size":1}}),
    );
    let parsed = odp_core::parse_agent_offering(&support::encode(&document)).unwrap();
    assert_eq!(parsed.id, "plant-1");
