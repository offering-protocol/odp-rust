...
pub trait Catalog: Send + Sync {
    ...
    /// Resumes a search using the opaque cursor, without reconstructing the original query.
    async fn continue_offering_search(
        ...
        Err(ServiceError::request(
            ...
            "Search continuation is unavailable",
        ))
    }

    /// Resumes a Collection search using the opaque cursor.
...
impl Service {
    ...
    pub async fn handle(&self, request: Request) -> Response {
        ...
        let retrieval = head || request.method.eq_ignore_ascii_case("GET");
        ...
        if response.status == 200 {
            if let Some(etag) = response.headers.get("etag").cloned() {
                if conditional.is_some_and(|value| matches_etag(&value, &etag)) {
                    response.body.clear();
                    response.status = if retrieval { 304 } else { 412 };
    ...
    async fn handle_result(&self, request: Request) -> Result<Response, ServiceError> {
        ...
        let mut input = catalog_request(&request, language.clone())?;
        if !url::form_urlencoded::parse(request.query.as_bytes())
            .any(|(key, _)| key == "representation")
            && matches!(
                path_operation(request.method.as_str(), path),
                Some(Operation::GetOffering | Operation::GetCollection)
            )
        {
            input.representation = Representation::Full;
        }
        ...
        match (request.method.as_str(), path) {
            ...
            ("GET", "/offerings/search") | ("GET", "/collections/search") => {
                if input.cursor.as_ref().is_none_or(String::is_empty) {
                    return Err(request_error(
                        400,
                        "INVALID_REQUEST",
                        "Search continuation requires a cursor",
                    ));
                }
                let representation = input.representation;
                if path == "/offerings/search" {
                    let page = self.catalog.continue_offering_search(input).await?;
                    validate_offering_page(&page, true, representation, limit, &request)?;
                    self.represent(200, &page, MAXIMUM_RESOURCE_BYTES, &language)
                } else {
                    let page = self.catalog.continue_collection_search(input).await?;
                    validate_collection_page(&page, representation, limit, &request)?;
                    self.represent(200, &page, MAXIMUM_RESOURCE_BYTES, &language)
                }
            }
...
fn catalog_request(request: &Request, language: String) -> Result<CatalogRequest, ServiceError> {
    ...
    let limit = single("limit")
        ...
        .unwrap_or(0);
    if limit > 100 || (limit == 0 && single("limit").is_some()) {
        return Err(request_error(
            400,
            "INVALID_REQUEST",
...
fn path_operation(method: &str, path: &str) -> Option<Operation> {
    match (method, path) {
        ...
        ("POST" | "GET", "/offerings/search") => Some(Operation::SearchOfferings),
        ("GET", "/collections") => Some(Operation::ListCollections),
        ("POST" | "GET", "/collections/search") => Some(Operation::SearchCollections),
...
fn json_response<T: serde::Serialize>(
    ...
    let mut document = serde_json::to_value(value)
        .map_err(|error| ServiceError::InvalidResponse(error.to_string()))?;
    odp_core::validate_json_depth(
        &document,
        if maximum_bytes == MAXIMUM_REQUEST_BYTES {
            8
        } else {
            16
        },
    ...
    .map_err(|error| ServiceError::InvalidResponse(error.to_string()))?;
    if let Some(version) = document.get_mut("odp_version") {
    ...
    {
        for item in items {
            if let Some(version) = item.get_mut("odp_version") {
                *version = serde_json::Value::String(VERSION.to_owned());
            }
        }
    }
...
fn allowed_methods(path: &str) -> &'static str {
    match path {
        "/offerings/search" | "/collections/search" => "POST, GET, HEAD",
        _ => "GET, HEAD",
    }
}
